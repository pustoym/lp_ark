...
const testInstruments = () => {
  ...
  panel.classList = "ts-panel ts-panel--close";
  ...
  image.onclick = () => {
    ...
    if (images.length !== 0) {
      ...
      con.textContent =
        con.textContent + "Количество картинок: " + imagesCount + "\r\n";
      con.textContent =
        con.textContent + "Количество picture: " + pictureCount + "\r\n";
      con.textContent =
        con.textContent + "Встречается кадрирование: " + mediaCount + "\r\n";
      con.textContent =
        con.textContent + "Количество webp: " + webpCount + "\r\n";
      con.textContent =
        con.textContent + "Количество ретинизации: " + retinaCount + "\r\n";
    }
  };
  ...
  panelBoard.onclick = (event) => {
    if (
      panelBoard.classList.contains("ts-panel--close") &&
      event.target == panelBoard
    ) {
  ...
  href.onclick = () => {
    ...
    b.forEach((item) => {
      if (!item.classList.contains("ts-btn")) {
        ...
        if (item.innerText == "") {
          con.textContent =
            con.textContent +
            "Контент кнопки: пустая." +
            item.classList +
            "\r\n";
        } else {
          con.textContent =
            con.textContent + "Контент кнопки: " + item.innerText + "\r\n";
        }
    ...
    con.textContent =
      con.textContent + "Количество ссылок " + hrefCount + "\r\n";
    con.textContent =
      con.textContent + "Количество пустых ссылок " + hrefEmptyCount + "\r\n";
    con.textContent =
      con.textContent + "Количество кнопок " + btnCount + "\r\n";
    con.textContent =
      con.textContent + "Количество кнопок без типа  " + btnTypeCount + "\r\n";
  };
};
